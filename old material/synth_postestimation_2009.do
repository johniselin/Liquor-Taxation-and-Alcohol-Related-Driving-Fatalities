...
local vars1 `vars_`a'' `a'_alcohol(1983) `a'_alcohol(1985) `a'_alcohol(1991) `a'_alcohol(1993) `a'_alcohol(1999)
local vars2 `vars_`a'' 
di "`vars1'"
di "`vars2'"
...
export excel using "`mypath'Analysis_`a'_`b'.xlsx", sheet("Pre-Treatment Test - Data") sheetmodify  firstrow(var)
clear
...
export excel using "`mypath'Analysis_`a'_`b'.xlsx", sheet("Leave-One-Out - Data") sheetmodify  firstrow(var)
clear
