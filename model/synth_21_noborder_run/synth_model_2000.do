...
cd "`mypath_nb'"
log using "`mypath_logs'synth_alcohol_model_nb_21plus_2000", replace
...
synth `a'_alcohol `a'_alcohol(1992) `a'_alcohol(1993) `a'_alcohol(1994) 		///
	`a'_alcohol(1995) `a'_alcohol(1996) `a'_alcohol(1997) `a'_alcohol(1998), 	///
	trunit(17) trperiod(2000)  xperiod(1992(1)1998) mspeperiod(1992(1)1998) 	///
	resultsperiod(1992(1)2009)  fig keep(all_lags) replace
graph export all_lags.pdf, replace 
clear

use all_lags
export excel using "`mypath_nb'Analysis_`a'_`b'_2000.xlsx", sheet("All Lags - Data") sheetmodify  firstrow(var)
drop if _time==.
...
foreach x of local states {
	...
	resultsperiod(1992(1)2009)  keep(`x'placebo_lags) replace
...
export excel using "`mypath_nb'Analysis_`a'_`b'_2000.xlsx", sheet("Placebo Lags - Data") sheetmodify  firstrow(var)
clear
...
synth `a'_alcohol `a'_alcohol(1992) `a'_alcohol(1993) `a'_alcohol(1994), ///
trunit(17) trperiod(1995) xperiod(1992(1)1998) mspeperiod(1992(1)1998) ///
	resultsperiod(1992(1)1998)  fig keep(1995_all_lags) replace 
...
use "`mypath_data'alcohol_noborders_21plus.dta"
tsset state year
drop if year != 2015
...
quietly: save "`mypath_data'donorstate_weights_nb_21plus_2000.dta", replace
