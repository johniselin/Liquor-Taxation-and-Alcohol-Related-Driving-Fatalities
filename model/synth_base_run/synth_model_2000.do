...
Note: Per referee request, this version shortens the pre-intervention time 
period to only those years after 1991, the year of the last federal excise tax
increase.
...
This do-file is the second of three for the first tax change. This file takes 
the datasets created in synth_preestimation and runs through our model. The 
model is run twice for each version of the data - once with all pre-treatment 
dependent variable lags, and once with our selected explanatory variables plus 
lags selected based on our preestimation tests. This file also applies our 
placebo tests, which runs each potential donor state through the same model that 
we apply to IL.
...
cd "`mypath'"
log using "`mypath_logs'synth_alcohol_model_2000", replace
