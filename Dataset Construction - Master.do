...
if regexm(c(os),"Mac") == 1 {
	local mypath = "/Users/johniselin/Box Sync/LiquorTax/Data/Dataset Construction/"
	}
	else if regexm(c(os),"Windows") == 1 local mypath = "D:\Users\JIselin\Box Sync\LiquorTax\Data\Dataset Construction\"

** FOR FINAL DATA --> Make sure that the filepath below exists for the final datasets
if regexm(c(os),"Mac") == 1 {
	local mypath_synth = "/Users/johniselin/Box Sync/LiquorTax/Data/Synth/"
	}
	else if regexm(c(os),"Windows") == 1 local mypath_synth = "D:\Users\JIselin\Box Sync\LiquorTax\Data\Synth\"
	

log using "`mypath'DatasetConstruction_Log.smcl", replace
cd "`mypath'"
...
do "Dataset Construction - FARS.do"
...
quietly: save "Dataset (1982-2015).dta", replace
...
quietly: save "Dataset US (1982-2015).dta", replace
clear
use "Dataset (1982-2015).dta", 
append using "Dataset US (1982-2015).dta"
quietly: save "Dataset (1982-2015).dta", replace
...
quietly: save "alcohol.dta", replace
quietly: save "`mypath_synth'alcohol.dta", replace
...
use "fars - no borders - 1982-2015.dta", clear
sort statename year
merge 1:1 statename year using "Age Brackets 1982-2015.dta"
drop _merge
merge 1:1 statename year using "State Motor Fuel Tax 1982-2015.dta"
...
quietly: save "Dataset - no borders - (1982-2015).dta", replace
...
quietly: save "alcohol_noborders.dta", replace
quietly: save "`mypath_synth'alcohol_noborders.dta", replace
*export excel "Dataset (1982-2015).xlsx", firstrow replace 
sum
de
